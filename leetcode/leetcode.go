...
func fetchLeetCode(username string) *Leetcode {
	problems, record := parseAlgorithms(fetchAlgorithms(username))
	lc := &Leetcode{
		Username: username,
		Record:   record,
		Problems: *problems,
...
func (lc *Leetcode) refresh() {
	...
	newLC := fetchLeetCode(lc.Username)
	logDiff(lc, newLC)
	*lc = *newLC
}
